...
description: '介绍 GoFrame 框架 grpcx 脚手架组件的完整功能，包括 Server 服务端对象的创建与维护、Client 客户端的服务名称访问、Ctx 上下文的 Incoming/Outgoing 数据传递、Balancer 负载均衡策略（随机、轮询等）、Resolver 服务注册发现（支持 etcd 等组件），帮助开发者快速构建和管理 gRPC 微服务应用。'
---
