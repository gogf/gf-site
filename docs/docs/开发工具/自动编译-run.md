...
description: 'GoFrame框架提供的自动编译运行命令工具，通过gf run命令实现类似热编译的开发体验。需要注意Golang本身不支持真正的热编译，gf run实际是通过监控项目go文件变化来自动触发重新编译和运行，从而避免手动停止、编译、启动的重复操作。工具会递归监控当前运行目录的所有go文件变化，支持自定义编译参数、运行参数、输出目录以及额外监控路径等配置，支持命令行和配置文件两种参数配置方式，显著提升本地开发调试效率。'
---
