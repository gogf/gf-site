...
sidebar_position: 6
hide_title: true
keywords: [ GoFrame,GoFrame框架,配置管理,接口化设计,高扩展性,配置监听,自定义回调,Watcher ]
description: 'WatcherAdapter 是 gcfg 包中的一个接口，用于实现配置文件变化监听功能。当配置文件被修改、创建、删除或权限发生变化时，注册的观察者会收到通知。'
---

## 基本介绍

从`v2.9.5`版本开始`gcfg` 组件新增`WatcherAdapter`接口， `WatcherAdapter` 是 `gcfg`
