...
description: '全面介绍`GoFrame`框架`gcmd`组件的常用基础方法，包括`Init`自定义命令行数据、`GetArg`/`GetArgAll`参数获取、`GetOpt`/`GetOptAll`选项获取以及`GetOptWithEnv`环境变量自动读取支持。这些方法按照默认解析规则直接获取命令行参数及选项，简化命令行操作。'
---
