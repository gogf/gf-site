...
sidebar_position: 4
hide_title: true
keywords: [GoFrame,错误处理,国际化,i18n,ITextArgs,多语言,错误翻译,gerror,错误消息,本地化]
description: '详细介绍`GoFrame`框架`gerror`组件的国际化支持功能（`v2.10.0+`），通过`ITextArgs`接口分离错误消息模板和参数，实现错误信息的多语言翻译。讲解`Text()`、`Args()`方法的使用，并提供国际化处理、结构化日志记录、错误聚合等实际应用场景示例。'
---
