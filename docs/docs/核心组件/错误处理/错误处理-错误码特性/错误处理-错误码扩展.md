...
description: '详细讲解`GoFrame`框架`gcode`组件的错误码扩展功能，通过`Detail`参数实现复杂业务场景下的错误码自定义。示例展示如何定义包含用户信息的`BizCode`结构体，使用`gcode.WithCode`方法创建带扩展数据的错误码。在中间件中获取错误码详情进行日志记录，并将错误信息返回给客户端，实现灵活的错误处理和日志管理。'
---
