...
description: '全面讲解`GoFrame`框架`gerror`组件中带错误码的`error`对象创建和操作方法。包括创建方法：`NewCode/NewCodef`用于创建带错误码和堆栈信息的错误对象；包裹方法：`WrapCode/WrapCodef`用于包裹现有错误并添加错误码；高级方法：`NewCodeSkip/WrapCodeSkip`可忽略部分堆栈信息；`Code`方法用于递归获取错误对象中的错误码。提供详细代码示例帮助开发者掌握错误码的实际应用。'
---
