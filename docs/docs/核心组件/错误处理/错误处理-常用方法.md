...
description: '深入介绍`GoFrame`框架`gerror`组件的常用方法，包括错误创建方法`New/Newf`（创建带堆栈信息的错误）、错误包装方法`Wrap/Wrapf`（构造多级错误链）、以及高级方法`NewSkip/NewSkipf`和`WrapSkip/WrapSkipf`（忽略部分堆栈信息），帮助开发者有效管理和调试错误。'
---
