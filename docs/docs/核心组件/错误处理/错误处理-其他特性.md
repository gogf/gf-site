...
description: '详细介绍`GoFrame`框架`gerror`组件的高级特性，包括使用`NewWithOption`自定义错误对象创建、`fmt`格式化符号（`%v`、`%+v`等）打印堆栈信息、`glog`日志组件对`gerror`的天然支持，以及通过环境变量`GF_GERROR_STACK_MODE`配置堆栈打印模式（简略/详细）。'
---
