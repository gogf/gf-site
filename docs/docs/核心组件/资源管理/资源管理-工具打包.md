...
description: '详细介绍如何使用`GoFrame`框架的`gf`命令行工具进行资源文件打包。通过`gf pack`命令可将项目的`config`、`public`、`template`等目录打包成`Go`文件，生成的文件会通过`gres.Add`方法将压缩后的`BASE64`字符串资源添加到默认资源管理器。详细讲解在`boot`包中引入`packed`资源包的正确顺序，以及在`main`包中引入`boot`包的方式。提供了`gres.Dump()`方法来打印资源文件列表以便验证打包结果。'
---
