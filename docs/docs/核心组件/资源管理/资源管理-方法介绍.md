...
description: '全面介绍`GoFrame`框架资源管理组件`gres`的常用方法。详细讲解`Add`方法用于添加压缩内容到资源对象，`Load`方法用于加载文件数据，`Get`方法返回指定路径文件，`GetWithIndex`用于目录索引文件搜索（适用于`HTTP`静态服务），`GetContent`直接返回文件内容，`Contains`检查资源是否存在，`ScanDir`等方法用于扫描目录中的文件。每个方法都提供了完整的函数签名和实际代码示例，帮助开发者掌握资源管理操作。'
---
:::tip
