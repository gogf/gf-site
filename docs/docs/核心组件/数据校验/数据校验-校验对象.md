...
description: '全面介绍`GoFrame`框架数据校验组件的`Validator`对象使用，包括校验对象的配置管理和便捷的链式操作。详细描述`New`、`Assoc`、`Bail`、`Ci`、`Run`等方法的使用，并通过单数据校验、`Struct`校验和`Map`校验的实际示例，帮助开发者快速掌握数据校验的使用技巧，支持自定义规则、错误提示和`I18N`国际化。'
---
