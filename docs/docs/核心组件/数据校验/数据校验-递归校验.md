...
description: '详细介绍GoFrame框架数据校验组件的递归校验（嵌套校验）特性。支持对struct、slice、map等复杂数据类型进行自动递归校验，通过多个实际示例演示如何对嵌套结构体、数组和映射类型数据进行有效验证。特别说明了空对象在递归校验中的处理规则，包括带有默认值的空结构体将被视为已传递对象并执行校验，而空指针对象则不会触发递归校验。'
---
