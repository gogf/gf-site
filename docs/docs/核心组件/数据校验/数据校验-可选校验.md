...
description: '详细讲解`GoFrame`框架数据校验组件中的可选校验规则使用方法。当校验规则中不包含`required*`规则时，对于`nil`或空字符串将忽略校验。通过空字符串、空指针属性、空整型属性和`map`传参四个实际代码示例，帮助开发者理解可选校验的处理逻辑及注意事项，需要特别注意`0`和`false`值仍会被校验。'
---
