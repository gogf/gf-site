...
description: '解决使用`GoFrame`框架进行数据校验时，`Struct`默认值对`required`规则的影响及其解决方案。提供三种解决方法：使用指针类型绕过默认值影响、使用组合校验规则弥补默认值影响、以及使用`Assoc`联合校验方法设置联合校验参数，确保校验准确性，特别适用于`Server`结构化`API`输入输出场景。'
---
