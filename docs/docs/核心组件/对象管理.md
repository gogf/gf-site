...
description: '介绍GoFrame框架的g模块对象管理功能，包括常用数据类型别名（Var、Map、Slice等）以及单例对象的获取方法。通过g.*方法可以方便地获取HTTP客户端、数据校验、配置管理、日志管理、模板引擎、WEB/TCP/UDP Server、数据库ORM、Redis客户端等常用对象，实现简便高效的对象管理。'
---
