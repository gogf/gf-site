...
description: '全面讲解`GoFrame`框架模板引擎的标签语法和使用方法。介绍如何通过`SetDelimiters`自定义模板闭合标签，使用`.`访问局部变量、`$`访问全局变量。详细说明`pipeline`管道机制在函数调用和值传递中的应用，`if...else`条件判断语句的多种写法，`range`标签用于遍历`slice`、`map`、`channel`等类型的数据。讲解`with`标签的重定向功能，`define`标签定义模板内容块，`template`标签实现模板嵌套和跨文件引用，以及`include`标签在模板中的使用差异。'
---
