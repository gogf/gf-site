...
description: '详细介绍`GoFrame`框架视图组件的配置管理功能。通过配置文件可以管理视图组件的模板文件搜索目录、默认解析文件、变量分隔符、`XSS`编码等配置项。支持单个和多个`View`对象配置，使用`g.View()`方法可自动获取对应配置。讲解了`SetConfigWithMap`方法的使用，可通过键值对设置特定配置。特别提示模板标签配置`delimiters`与模板中使用的标签必须匹配的重要注意事项。'
---
