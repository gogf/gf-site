...
description: '详细介绍`GoFrame`框架日志组件的链式操作功能，支持通过一系列链式方法灵活设置日志输出参数。包括设置输出目录、文件分类、日志级别、开启堆栈打印、终端输出、异步输出等功能。通过多个实际代码示例展示了如何使用`Path`、`Cat`、`Line`、`Skip`等链式方法组合实现灵活的日志管理，包括文件回溯设置、调用行号打印等高级功能。'
---
