...
description: '介绍`GoFrame`框架日志组件的异步输出功能，用于提高日志打印效率。详细讲解两种设置异步输出的方式：通过`SetAsync`方法对日志对象设置全局异步输出，或通过`Async`链式方法对单个日志调用设置异步输出。异步输出使用`goroutine`池管理，可充分降低资源占用，但需注意避免混用同步和异步输出到同一文件以防日志乱序。'
---
