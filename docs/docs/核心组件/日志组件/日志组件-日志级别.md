...
description: '详细介绍`GoFrame`框架日志组件的日志级别管理功能。讲解三种设置日志级别的方法：`SetLevel`通过常量值设置，支持位操作组合；`SetLevelStr`通过字符串设置，支持多种级别名称包括`ALL`、`DEV`、`PROD`等；`SetLevelPrint`控制是否打印级别标识。日志级别按照`DEBU < INFO < NOTI < WARN < ERRO < CRIT`排序，过滤等级较低的日志输出。'
---
