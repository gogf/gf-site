...
description: '介绍`GoFrame`框架日志组件中如何设置日志文件的名称和输出目录。通过`SetFile`方法可以自定义日志文件名称格式，支持`gtime`时间格式变量，方便按日期分割日志文件。通过`SetPath`方法可以指定日志输出目录，利用`gfpool`文件指针池实现高效的文件写入操作。默认日志文件名称格式为`YYYY-MM-DD.log`，目录不存在时会自动递归创建。'
---
