...
description: '详细介绍`GoFrame`框架日志组件的颜色打印功能，通过为不同日志级别添加颜色突出显示，提高日志的可读性。默认情况下终端自带颜色输出，文件日志不带颜色。讲解如何通过配置文件中的`stdoutColorDisabled`和`writerColorEnable`选项，或者通过代码中的`SetWriterColorEnable`方法来启用文件日志颜色输出。提供了不同日志级别（`Debug`、`Info`、`Notice`、`Warning`、`Error`等）的默认颜色对应表。'
---
