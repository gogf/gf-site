...
description: '全面介绍`GoFrame`框架`gconv`包的基本类型转换功能。讲解如何使用`gconv`进行整型（`Int`、`Int8`、`Int16`等）、无符号整型（`Uint`系列）、浮点型（`Float32`、`Float64`）、布尔型、字符串以及数组切片类型的转换。通过完整代码示例展示各种转换方法的使用和输出结果。特别说明数字转换方法支持自动识别十六进制和八进制字符串，如`0x`开头的十六进制数字转换功能。'
---
